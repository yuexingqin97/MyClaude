...
- 用户对已有笔记**提问**：读取笔记内容，回答疑问，并将问答记录追加回原文档
...
- **本地文件模式**：输入为本地文件路径
  ...
  - 本地**文档**文件（`.md`、`.txt`、`.pdf`、`.html`、`.rst` 等）→ 直接读取内容，按 [本地文档模式](#本地文档模式) 处理
  - 特别支持：`.html` 文件（如手动下载的知乎/微信文章）→ python 提取纯文本后处理
- **本地目录模式**：输入为本地目录路径
...
- **问答模式**：用户对已有笔记提问 → 跳转到 [问答模式](#问答模式)
  - 触发词：`/myriad-mind ask 文档路径 问题`、`/myriad-mind 问`、`问一下 文档路径`、`提问`、`ask`
  - 识别逻辑：输入中包含 `ask` / `问` / `提问` + 指向已有笔记文件的路径
...
| 平台 | 状态 | 原因 | 替代方案 |
| --- | --- | --- | --- |
| **知乎** | ❌ 无法抓取 | JS Challenge (zse-ck) 反爬 | 提示用户复制粘贴文章内容 |
| **微信公众号** | ❌ 无法抓取 | 需要微信内置浏览器 Cookie | 提示用户复制粘贴文章内容 |
| **CSDN** | ⚠️ 有限支持 | 部分文章有验证码 | curl 尝试，失败则提示粘贴 |
| **掘金** | ✅ 通常可用 | — | WebFetch |
| **Wiki/Wikipedia** | ✅ 可用 | — | WebFetch |

**抓取失败时的自动降级流程：**

```text
⚠️ 无法自动抓取 {平台} 文章（平台反爬保护）。请选择：

方案 A（推荐）：在浏览器打开文章 → 右键"另存为" → 保存为 HTML 文件
          → 将 .html 文件路径发给我 → 我会自动提取正文生成笔记

方案 B：直接将文章内容复制粘贴到聊天中，我会立即为你生成学习笔记。
```

**方案 A 操作指南（各浏览器通用）：**
1. 在浏览器中打开目标文章
2. 右键页面空白处 → **"另存为..."**（或 `Ctrl+S`）
3. 保存类型选 **"网页，全部"** 或 **"网页，仅 HTML"**
4. 将保存的 `.html` 文件路径发给 Skill（如 `C:\Users\xxx\Desktop\文章标题.html`）

Skill 会自动：
- 用 python 解析 HTML，剔除 script/style/nav 等噪声
- 提取正文内容（支持 UTF-8 / GBK / GB2312 编码）
- 按正常文章模式生成学习笔记

> 💡 此流程已验证可用于知乎、微信公众号等 JS 反爬平台。HTML 文件可保留在本地，方便后续复用。
...
当用户对已有笔记内容有疑问时，读取笔记（及关联的原始素材），回答用户问题，并将问答记录追加到原笔记末尾。

### 触发方式

```
/myriad-mind ask 笔记路径 你的问题
/myriad-mind 问 LearnEcs_v2.md ECS 中 Commands 和 World 的区别是什么？
问一下 Bevy学习笔记/LearnEcs.md 什么是 QueryFilter？
我对这篇笔记有个问题：大衍决残卷/xxx.md 这里说的 Disabled 组件怎么用？
```

### 识别逻辑

- 输入中包含 `ask` / `问` / `提问` / `请教` / `有个问题` 等关键词
- **且**关键词后面紧跟或附近有指向已有 `.md` 笔记的路径
- 路径可以是绝对路径、相对路径、或仅文件名（在 `NOTE_OUTPUT_DIR` 下搜索）

### QA1. 定位目标文档

```bash
NOTE_OUTPUT_DIR="${NOTE_OUTPUT_DIR:-$(pwd)}"
TARGET_DOC="$1"

# 若路径不存在，在 NOTE_OUTPUT_DIR 下递归搜索同名文件
if [ ! -f "$TARGET_DOC" ]; then
  TARGET_DOC=$(find "$NOTE_OUTPUT_DIR" -name "$(basename "$TARGET_DOC")" -type f 2>/dev/null | head -1)
fi

if [ -z "$TARGET_DOC" ] || [ ! -f "$TARGET_DOC" ]; then
  echo "❌ 找不到笔记：$1"
  echo "   提示：请提供笔记的文件名或路径，例如 /myriad-mind ask LearnEcs.md 问题"
  exit 1
fi

echo "📖 目标笔记：$TARGET_DOC"
```

### QA2. 读取笔记并提取上下文

读取目标文档的全部内容。同时检查文档元信息中的 `原始资源` 字段——如果是视频/文章生成的笔记，原始 URL 可用于补充上下文。

**上下文优先级：**
1. 文档正文（核心概念、详细笔记、术语表等章节）
2. 文档元信息中的原始资源链接（若视频/文章仍可访问，可 WebFetch 补充）
3. 文档中的截图描述（截图文件本身可能在 `/tmp/` 已被清理，但截图标题和注释仍可参考）
...
- **答案质量要求：**
  - 先判断问题是否在文档覆盖范围内。若超出范围（文档未涉及），诚实告知，不要编造
  - 引用文档中的具体章节和内容，标注出处（如"见第二节 Query 部分"）
  - 若文档中有 Mermaid 图表能辅助解释，提及图表位置
  - 若原始视频/文章仍可访问且文档信息不足，可补充参考
  - 回答简洁但有深度，不要复述整篇笔记
...
📖 答：[基于笔记内容的回答]
   ...
   📍 参考：笔记「三、详细笔记 - Query 与 System 参数」章节
   📍 文档：[笔记文件名](路径)
```

### QA4. 追加问答记录到原文档

将问答内容追加到目标文档末尾（在 `文档元信息` 和 `调试信息` 之后，或新建 `问答记录` 章节）。

**追加规则：**

1. 若文档已有 `## 问答记录` 章节 → 在该章节末尾追加新条目
2. 若文档无此章节 → 在文档末尾（元信息之后）新建 `## 问答记录` 章节，然后追加

**条目格式：**

```markdown
### 💬 2026-05-31 14:30 — [用户问题摘要，不超过 30 字]
...
# 使用 cat << 'EOF' >> 追加到笔记末尾
# 先检查是否已有「问答记录」章节
if grep -q "^## 问答记录" "$TARGET_DOC"; then
  # 已有章节：在当前最后一个问答条目后追加
  # 找到最后一个 --- 分隔线后插入
  sed -i '/^## 文档元信息\|^> 🔧 调试信息/,$d' "$TARGET_DOC"  # 先移除元信息尾页
  # 追加新问答
  cat >> "$TARGET_DOC" << 'QAEOF'
[问答条目内容]
QAEOF
  # 重新追加元信息
else
...
> 💬 以下为学习过程中对本文档的提问与解答，由 AI 基于笔记内容生成。
...
> ⚠️ 建议：由 Claude 直接用 Edit 工具追加，而非通过 Bash 脚本。Claude 可以先 Read 文档末尾，判断是否需要新建章节，然后精确 Edit 插入。

### QA5（可选）. 关联到修为面板

若用户配置了 `AUTO_UPDATE_PANEL=true`，在问答记录追加后，可在修为面板中标记该笔记"最近有互动"，但不强制刷新完整面板。
